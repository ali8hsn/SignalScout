...
Operator digest view: generate/send controls and the full entry list.

- `Digest()` — loads the latest digest via `api.latestDigest()` through the shared `useAsyncData` hook (its `setData`/`reload` back the GENERATE/regenerate flow and the load-error TRY AGAIN), provides GENERATE (`api.generateDigest()`) and SEND TO SUBSCRIBERS (`api.sendDigest()`, a real send to all active subscribers with a sent/subscriber-count receipt) buttons, and renders `DigestSignup` plus each digest entry (name, score, thesis, top signals, orbit/intro context, why-now, and `ContactLinks`).
