...
- Data sources include GitHub, Twitter/X, USACO competitions, Semantic Scholar (co-authors), and Devpost hackathons; typed relationships are stored in the `graph_edges` table, with edge types such as `co_contributor`, `org_mate`, `co_author`, and `hackathon_teammate`.
...
- Enrichment is pluggable via `ENRICHMENT_PROVIDER` (default `pdl` = People Data Labs; also Coresignal); LinkedIn/contact data comes from these licensed providers, not from scraping.
